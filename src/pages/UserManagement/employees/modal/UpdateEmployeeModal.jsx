...
const UpdateEmployeeModal = ({isOpen,onClose , onSuccess , data}) => {
  ...
  const { 
    ...
    const initialRoleIds = useMemo(() => {
      if (!roles.length || !data.roles) return [];
      return roles
    ...
    const initialDepartmentId = useMemo(() => {
        const foundDept = departments.find(d => d.label === data.departmentName);
        return foundDept ? foundDept.value : null; 
  ...
  const handleSubmit = async (e) => {
    ...
    // استدعاء السيرفس
    const response = await updateEmployee(data.id,userPayload, token);
      ...
      <form onSubmit={handleSubmit} className="bg-[#171A21] rounded-xl border border-[#2A2E37] p-10 w-full max-w-2xl flex flex-col gap-6 shadow-3xl max-h-[80vh] overflow-auto custom-scrollbar">
        ...
        <div className="flex justify-between items-center px-8 py-6">
          <h2 className="text-2xl font-semibold text-white">
            Update Employee <span className='text-sky-600 uppercase '>{data.firstName} {data.lastName} </span> Details
          </h2>
        </div>
        ...
        <div className="px-8 pb-8 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
          ...
          <div className="space-y-6">
            ...
            <div>
               ...
               <label className=" block mb-2 text-xs font-bold uppercase text-slate-400 ">Roles</label>
