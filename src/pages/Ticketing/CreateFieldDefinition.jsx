...
const CreateFieldDefinition = ({ inSystem }) => {
  ...
  // تعريف الـ Enums الخاصة بالصلاحيات
  const AccessRules = [
    { value: "EVERYONE", label: "Everyone (الكل)" },
    { value: "NO_ONE", label: "No One (النظام فقط)" },
    { value: "TICKET_RESOLVER_ONLY", label: "Ticket Resolver Only (الموظف المعين فقط)" },
    { value: "TEAM_ONLY", label: "Team Only (فريق العمل)" },
    { value: "MANAGERS_ONLY", label: "Managers Only (المدراء والمشرفون)" },
    { value: "CUSTOMER_ONLY", label: "Customer Only (العميل فقط)" }
  ];
  ...
  const handleCreateField = async () => {
    try {
      const payload = {
        ...
        readRule: formData.readRule,
        writeRule: formData.writeRule,
  ...
  return (
    <div className={` ${!inSystem ? `min-h-screen bg-gray-900 flex items-center justify-center p-4 font-sans` : `min-h-screen bg-gray-900 flex p-4 font-sans`} `}>
      <div className={`bg-[#1a2332] ${!inSystem ? `w-full max-w-3xl ` : `w-full`} rounded-xl shadow-2xl border border-gray-800`}>
        ...
        <div className="p-6 border-b border-gray-800 flex justify-between items-center">
          <div>
            ...
            <div className={`transition-all duration-300 ease-linear overflow-hidden ${isMore ? 'opacity-100 max-h-40' : 'opacity-0 max-h-0'}`}>
              <div className='mt-2'>
        ...
        <div className="p-8 space-y-6">
          ...
          {/* إضافة خيارات Read Rule و Write Rule */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase mb-2">Read Rule</label>
              <div className="relative">
                <select 
                  name="readRule"
                  value={formData.readRule}
                  onChange={handleInputChange}
                  className="w-full bg-[#252f3f] border border-transparent focus:border-blue-500 rounded-md py-3 px-4 text-gray-200 outline-none appearance-none cursor-pointer text-sm"
                >
            ...
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase mb-2">Write Rule</label>
              <div className="relative">
                <select 
                  name="writeRule"
                  value={formData.writeRule}
                  onChange={handleInputChange}
                  className="w-full bg-[#252f3f] border border-transparent focus:border-blue-500 rounded-md py-3 px-4 text-gray-200 outline-none appearance-none cursor-pointer text-sm"
                >
                  {AccessRules.map(rule => (
